...
from boards.lcd_api import LcdApi
...
class GpioLcd(LcdApi):
    ...
    def __init__(self, rs_pin, enable_pin, d0_pin=None, d1_pin=None,
        ...
        if d4_pin and d5_pin and d6_pin and d7_pin:
            ...
            self.d6_pin = d2_pin
            self.d7_pin = d3_pin
        self.rs_pin = Pin(self.rs_pin, Pin.OUT)
        self.rs_pin.value(0)
        if self.rw_pin:
            self.rw_pin = Pin(self.rw_pin, Pin.OUT)
            self.rw_pin.value(0)
        self.enable_pin = Pin(self.enable_pin, Pin.OUT)
        self.enable_pin.value(0)
        self.d4_pin = Pin(self.d4_pin, Pin.OUT)
        self.d5_pin = Pin(self.d5_pin, Pin.OUT)
        self.d6_pin = Pin(self.d6_pin, Pin.OUT)
        self.d7_pin = Pin(self.d7_pin, Pin.OUT)
        self.d4_pin.value(0)
        self.d5_pin.value(0)
        self.d6_pin.value(0)
        self.d7_pin.value(0)
        if not self._4bit:
            self.d0_pin = Pin(self.d0_pin, Pin.OUT)
            self.d1_pin = Pin(self.d1_pin, Pin.OUT)
            self.d2_pin = Pin(self.d2_pin, Pin.OUT)
            self.d3_pin = Pin(self.d3_pin, Pin.OUT)
            self.d0_pin.value(0)
            self.d1_pin.value(0)
            self.d2_pin.value(0)
            self.d3_pin.value(0)
        if self.backlight_pin is not None:
            self.backlight_pin = Pin(self.backlight_pin, Pin.OUT)
            self.backlight_pin.value(0)

        # See about splitting this into begin
        utime.sleep_us(20) # Allow LCD time to powerup
        # Send reset 3 times
        self.hal_write_init_nibble(self.LCD_FUNCTION_RESET)
        utime.sleep_us(5)    # need to delay at least 4.1 msec
        self.hal_write_init_nibble(self.LCD_FUNCTION_RESET)
        utime.sleep_us(1)
        self.hal_write_init_nibble(self.LCD_FUNCTION_RESET)
        utime.sleep_us(1)
    ...
    def hal_pulse_enable(self):
        ...
        self.enable_pin.value(0)
        utime.sleep_us(50)     # Commands need > 37us to settle
