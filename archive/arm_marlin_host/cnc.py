...
def parse_move_html(cmd):
    cmd = cmd.split('?')[1]
    cmd = cmd.split('&')
    cmd = [c.split('=') for c in cmd]
    cmd = [[c[0].upper(), c[1]] if c[1] != '' else '' for c in cmd]
    cmds = []
    for c in cmd:
        if c != '':
            cmds.append(str(c[0] + c[1]))
    line = 'G1 {} F2000'.format(' '.join(cmds))
    return line
...
class GRBL:
    def __init__(self, sd, can, uart, hbt_int):
        ...
        self.states = {'idle': self.idle, 'running': self.running}
